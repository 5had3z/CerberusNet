...
class SegmentationNet3(nn.Module):
    ...
    Module for extracting semantic segmentation from encoding pyramid
    '''
    def __init__(self, input_ch, num_classes, interm_ch=32):
        super(SegmentationNet3, self).__init__()
        self.classifier = Classifer(interm_ch, num_classes)

        self.feature_fusion = nn.ModuleList([LinearBottleneck(input_ch[0], interm_ch, t=1, stride=1)])
        for ch_in in input_ch[1:]:
            self.feature_fusion.append(LinearBottleneck(ch_in+interm_ch, interm_ch, t=1, stride=1))
...
class MonoSFNet(nn.Module):
    ...
    def __init__(self, upsample=True):
        ...
        self.segmentation_network = SegmentationNet3(num_chs[:0:-1], 19)

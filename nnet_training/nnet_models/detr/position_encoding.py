...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, features: torch.Tensor):
        ...
        del mask_size[1]
        dummy_mask = torch.ones(mask_size, dtype=torch.bool, device=features.device)
        y_embed = dummy_mask.cumsum(1, dtype=torch.float32)
        x_embed = dummy_mask.cumsum(2, dtype=torch.float32)
        if self.normalize:
            eps = 1e-6

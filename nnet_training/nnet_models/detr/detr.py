...
class Detr(nn.Module):
    ...
    def forward(self, samples: NestedTensor):
        """ The forward expects a NestedTensor, which consists of:
               ...
               - samples.mask:      a binary mask of shape [batch_size x H x W], containing 1 on padded
                                    pixels
...
class DetrHead(nn.Module):
    """ This is the DETR module that performs object detection """
    def __init__(self, num_channels, transformer, num_classes, num_queries, aux_loss=False):
        """ Initializes the model.
        Parameters:
...
class PostProcess(nn.Module):
    ...
    @staticmethod
    @torch.no_grad()
    def forward(outputs, target_sizes):
        """ Perform the computation
        Parameters:
            outputs: raw outputs of the model
            target_sizes: tensor of dimension [batch_size x 2] containing the size of each
                          images of the batch. For evaluation, this must be the original image
                          size (before any data augmentation). For visualization, this should
                          be the image size after data augment, but before padding
        """
...
def build(args):
    ...
    if args.masks:
        losses += ["masks"]
    criterion = DetrLoss(num_classes, matcher=matcher, weight_dict=weight_dict,

...
"""
This file provides the definition of the convolutional heads used to predict
masks, as well as the losses.
"""
import io
...
class DETRsegm(nn.Module):
    """
    DETR head with panoptic/instance segmentation
    """
    ...
    def forward(self, samples: NestedTensor):
        ...
        outputs_seg_masks = seg_masks.view(
            features[-1].tensors.shape[0], self.detr.num_queries,
...
class PostProcessSegm(nn.Module):
    ...
    def forward(self, results, outputs, orig_target_sizes, max_target_sizes):
        ...
        outputs_masks = outputs["masks"].squeeze(2)
        outputs_masks = F.interpolate(
            outputs_masks, size=(max_h, max_w), mode="bilinear", align_corners=False)
        outputs_masks = (outputs_masks.sigmoid() > self.threshold).cpu()

        for i, (cur_mask, t, tt) in \
                enumerate(zip(outputs_masks, max_target_sizes, orig_target_sizes)):
            img_h, img_w = t[0], t[1]
            results[i]["masks"] = cur_mask[:, :img_h, :img_w].unsqueeze(1)
            results[i]["masks"] = F.interpolate(
                results[i]["masks"].float(), size=tuple(tt.tolist()), mode="nearest").byte()
...
class PostProcessPanoptic(nn.Module):
    ...
    def __init__(self, is_thing_map, threshold=0.85):
        """
        Parameters:
           is_thing_map: This is a whose keys are the class ids, and the values a boolean indicating
                          whether the class is  a thing (True) or a stuff (False) class
    ...
    def forward(self, outputs, processed_sizes, target_sizes=None):
        ...
        Parameters:
            outputs: This is a dict coming directly from the model. See the model doc for the
                     content.
            ...
            target_sizes: This is a list of tuples (or torch tensors) corresponding to the
                          requested final size of each prediction. If left to None, it will
                          default to the processed_sizes.
            """
